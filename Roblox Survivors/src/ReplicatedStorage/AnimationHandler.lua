...
function AnimationHandler.getAnimationFolder(category, specificType)
	local animationsFolder = replicatedStorage:FindFirstChild(category.."Animations")
	...
	if specificType then
		local prefix = specificType:match("(.*)Zombie") -- catches the suffix
		local suffix = specificType:match("(Zombie)$")
...
function AnimationHandler.playAnimation(animationTable)
	local animator = animationTable.animator
	...
	for name, animtrack in pairs(animations) do
		local track = AnimationHandler.getAnimationTrack(animator.Parent, category, name)
		if string.match(name, "^Attack") then
	...
	local IKControl = animator.Parent:FindFirstChild("IKControl")
	local IKLeft = animator.Parent:FindFirstChild("IKLeft")
	local IKRight = animator.Parent:FindFirstChild("IKRight")
...
function AnimationHandler.monitorMovement(animationTable)
	...
	local humanoid = animationTable.animator
	local runSpeed = humanoid:GetAttribute("RunSpeed") or 25
	local walkSpeed = humanoid:GetAttribute("WalkSpeed") or 16
...
local baseplate = workspace.Map:WaitForChild("Baseplate")  -- The tilted baseplate
