...
function PlayerStats.initializeStats(player, profile)
	...
	local weapon = profile.Data.Weapon
...
function PlayerStats.add(player, object, amount)
	...
	if object == "Experience" then
		...
		if storedObjectAmount >= expThreshold then
			character:SetAttribute("Experience", storedObjectAmount - expThreshold) -- Carry over excess experience
			character:SetAttribute("Level", level + 1) -- Level up			
